...
| `yolov8n-detector.json` | **YOLOv8n** object detector (640×640, 233 nodes) — the model most people reach for, **MLX 5.16× faster than CPU** | `ep.claim` shows **233/233 nodes claimed (100%) → 1 fused subgraph** (the whole detector, incl. the DFL non-last-axis `Softmax`, runs as a single compiled MLX closure); `mlx.compute[general]` cache MISS→HIT across the 3 runs; `mlx.session_summary` at-a-glance |
| `fcn-resnet50-segmentation.json` | **FCN-ResNet50** semantic segmentation with **dynamic spatial dims** (`[-1,C,-1,-1]`), 139 nodes — **MLX 8.95× faster** (was 0.87×, i.e. *slower*, before dynamic-spatial Conv/MaxPool claiming) | `ep.claim` **137/139 claimed (98.6%) → 1 fused subgraph** (only 2 `Resize` → CPU); shows the shape-keyed general-compile path resolving the concrete image size at trace time; the win that flips a fragmented 52-subgraph graph into one |
| `resnet18-classifier.json` | **ResNet18** image classifier (224×224, timm export, 49 nodes) — clean CNN, MLX ~2–3× faster | `ep.claim` **49/49 claimed (100%) → 1 fused subgraph** (BatchNorm folded into Conv by the timm export); a compact reference trace for a fully-claimed feed-forward CNN |
